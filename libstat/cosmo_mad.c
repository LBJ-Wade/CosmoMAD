...
{
  return pow(mass/(CSM_FOURPITHIRD*(par->bg->OM)*2.77459457E11),
	     0.3333333333333333);
}

double csm_R2M(Csm_params *par,double radius)
{
  return CSM_FOURPITHIRD*(par->bg->OM)*
    2.77459457E11*radius*radius*radius;
}
...
double csm_cfrac(double nu,char *mf_model)
{
  ...
  if(!strcmp(mf_model,"PS")) model=0;
  else if(!strcmp(mf_model,"JAP")) model=1;
  else if(!strcmp(mf_model,"ST")) model=2;
  else {
    fprintf(stderr,"CosmoMad: wrong mass-function model %s.\n",mf_model);
    exit(1);
  }

  if(model==0) return erfc(nu*M_SQRT1_2);
  else if(model==1) {
    return exp(-0.412*nu*nu)/(1+1.529*pow(nu,0.704));
  }
  else if(model==2) {
    return 0.322184*(erfc(0.594559*nu)+
	      0.458264*gsl_sf_gamma_inc(0.2,0.3535*nu*nu));
  }
}

double csm_collapsed_fraction(Csm_params *par,double mass,char *mf_model)
{
  double rad=csm_M2R(par,mass);
  double sM=sqrt(csm_sig0_L(par,rad,rad,"TopHat","TopHat"));
  double nu=CSM_DELTA_C/sM;

  return csm_cfrac(nu,mf_model);
}
